...
from aliquotmaf.subcommands.utils import assert_sample_in_header, extract_annotation_from_header
...
def test_extract_annotation_from_header(get_test_vcf_header):
    hdr_meta = {
      ...
      'items': [
          ...
          ('Number', '.'),
          ('Type', 'String'),
          ('Description', "Consequence annotations from Ensembl VEP. Format: ItemA|ItemB")]}
    ...
    with pytest.raises(AssertionError):
        ann_cols_format, vep_key = extract_annotation_from_header(vcf_object, vep_key='CSX')
    ...
    hdr_meta = {
      ...
      'items': [
          ...
          ('Number', '.'),
          ('Type', 'String'),
          ('Description', "Consequence annotations from Ensembl VEP. Format: ExAC_MAF|ExAC_Adj_MAF|ExAC_AFR_MAF|ExAC_AMR_MAF|ExAC_EAS_MAF|ExAC_FIN_MAF|ExAC_NFE_MAF|ExAC_OTH_MAF|ExAC_SAS_MAF")]}

    vcf_object = get_test_vcf_header(meta=hdr_meta) 
    ann_cols_format, vep_key = extract_annotation_from_header(vcf_object, vep_key='CSQ')
    assert ann_cols_format == ['ExAC_AF', 'ExAC_AF_Adj', 'ExAC_AF_AFR', 
        'ExAC_AF_AMR', 'ExAC_AF_EAS', 'ExAC_AF_FIN', 'ExAC_AF_NFE', 
        'ExAC_AF_OTH', 'ExAC_AF_SAS']
    ...
    hdr_meta = {
      ...
      'items': [
          ...
          ('Number', '.'),
          ('Type', 'String'),
          ('Description', "Consequence annotations from Ensembl VEP. Format: ExAC_MAF|ExAC_ERROR")]}

...
from aliquotmaf.subcommands.vcf_to_protected.extractors.effects import EffectsExtractor, SelectOneEffectExtractor
    ...
    for i in effects:
        for j in biotypes:
            ...
            for col in ANNO_COLUMNS: 
                if col == 'Consequence': rec.append(i)
                elif col == 'BIOTYPE': rec.append(j)
                elif col == 'Allele': rec.append(allele)
                elif col == 'ALLELE_NUM': rec.append(allele_num)
                else: rec.append("")

            lst.append(rec)
    return lst 

@pytest.mark.parametrize("effect_priority, biotype_priority, var_idx, enst, expected", [
    ({"A": 1, "B": 2, "C": 3, "": 10}, {"A": 1, "B": 2, "C": 3}, 1, None, ("A", "A")),
    ({"A": 1, "B": 2, "C": 3, "": 10}, {"A": 2, "B": 1, "C": 3}, 1, None, ("A", "B")),
    ({"A": 1, "B": 2, "C": 3, "": 10}, {"A": 2, "B": 3, "C": 1}, 1, None, ("A", "C")),
    ({"A": 3, "B": 1, "C": 3, "": 10}, {"A": 1, "B": 2, "C": 3}, 1, None, ("B", "A")),
    ({"A": 1, "B": 1, "C": 3, "": 10}, {"A": 2, "B": 1, "C": 3}, 1, None, ("A", "B")),
])
def test_select_one_effect_extractor(effect_priority, biotype_priority, var_idx, enst, expected):
    """
    ...
    res = EffectsExtractor.extract(effect_priority, biotype_priority, ANNO_COLUMNS, elist, var_idx)
    all_effects, selected = SelectOneEffectExtractor.extract(res, effect_priority, biotype_priority, custom_enst=enst) 

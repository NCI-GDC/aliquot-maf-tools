...
def test_cosmic_snp(test_scheme, setup_annotator, get_test_file, 
    ...
    ## setup 
    vcf_record = get_test_vcf_record(chrom=record.chrom, pos=record.pos, alleles=record.alleles, ref=record.ref, alts=record.alts)
...
def test_cosmic_ins(test_scheme, setup_annotator, get_test_file, 
                    get_empty_maf_record, vcf_gen, get_test_vcf_record):
    vcf_path = get_test_file('ex2.vcf.gz')
    annotator = setup_annotator(test_scheme, source=vcf_path) 

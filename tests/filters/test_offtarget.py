...
@pytest.mark.parametrize("vcf_region, end_position, expected", [
  ('chr1:11:.:.:.', 11, True),
...
def test_offtarget_filter(test_scheme, setup_filter, get_test_file, get_empty_maf_record, 
                             vcf_region, end_position, expected):
    """
    Test offtarget filter 
    """
    bed_file = [get_test_file("fake_regions.bed.gz"), get_test_file("fake_regions_2.bed.gz")]
    filterer = setup_filter(bed_file)

...
subpops = ['nontcga_ExAC_AF_Adj', 'nontcga_ExAC_AF', 'nontcga_ExAC_AF_AFR', 'nontcga_ExAC_AF_AMR',
...
def test_exac_filter_4(test_scheme, setup_filter, get_empty_maf_record):
    ...
    for key in subpops:
        maf_record[key] = get_builder(key, test_scheme, value=0.0004) 
    maf_record['nontcga_ExAC_AF_Adj'] = get_builder('nontcga_ExAC_AF_Adj', test_scheme, value=0.00041)
    result = filterer.filter(maf_record)

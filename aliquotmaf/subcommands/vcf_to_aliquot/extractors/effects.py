...
class EffectsExtractor(Extractor):
    ...
    def extract(
        ...
        for edat in effect_list:
            ...
            if effect["HGVSc"]:
                effect["HGVSc"] = re.sub(r"^.*:", "", effect["HGVSc"])
            if effect["HGVSp"]:
                effect["HGVSp"] = re.sub(r"^.*:", "", effect["HGVSp"])
            ...
            effect["One_Consequence"] = (
                sorted(
                    effect["Consequence"].split(";"),
                    key=lambda x: effect_priority.get(x, 20),
                )[0]
            ...
            if effect["Existing_variation"]:
                ...
                effect["dbSNP_RS"] = ";".join(
                    [
                        i
                        for i in effect["Existing_variation"].split(";")
                        if re.search(r"^rs\d+$", i)
                    ]
                )
                if effect["dbSNP_RS"] == "":
                    effect["dbSNP_RS"] = None
            else:
...
class EffectsExtractor_102(EffectsExtractor):
    """A `~maf_converter_lib.extractor.Extractor` class that takes the VEP
    annotated data in formats into a list of dictionaries. This is specific
    to VEP v102 outputs and the GDC parameters.
    """
...
class SelectOneEffectExtractor(Extractor):
    ...
    def extract(cls, all_effects, effect_priority, biotype_priority, custom_enst=None):
        ...
        all_effects = sorted(
            all_effects,
            key=lambda x: (
                biotype_priority.get(x["BIOTYPE"], 20),
        ...
        try:
            maf_effect_with_gene_name = list(
                filter(lambda x: x["SYMBOL"], all_effects)
            )[0]
        except IndexError:
            maf_effect_with_gene_name = None
        maf_gene = (
            maf_effect_with_gene_name["SYMBOL"]
            if maf_effect_with_gene_name is not None
            else ""
        )
        ...
        if not maf_effect:
            ...
            if not maf_effect:
                try:
                    maf_effect = list(
                        filter(lambda x: x["CANONICAL"] == "YES", all_effects)
                    )[0]
                except IndexError:
                    maf_effect = None

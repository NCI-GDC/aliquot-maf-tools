"""
Main logic for merging protected MAFs on schema gdc-1.0.0-merged-protected.
"""
import json
...
from aliquotmaf.subcommands.merge_protected.runners import BaseRunner

from aliquotmaf.converters.utils import get_columns_from_header
from aliquotmaf.converters.builder import get_builder


class GDC_1_0_0_Merged_Protected(BaseRunner):
    def __init__(self, options=dict()):
        super(GDC_1_0_0_Merged_Protected, self).__init__(options)

        # Schema
        self.options['version'] = 'gdc-1.0.0'
        self.options['annotation'] = 'gdc-1.0.0-merged-protected'
    ...
    def __add_arguments__(cls, parser):
        ...
        parser.add_argument('--varscan2', help='Path to input protected VarScan2 MAF file')
        parser.add_argument('--somaticsniper', help='Path to input protected SomaticSniper MAF file')
        parser.add_argument('--pindel', help='Path to input protected Pindel MAF file')
        parser.add_argument('--min_n_depth', type=int, default=7, 
    ...
    def load_readers(self):
        ...
        for maf_key in maf_keys:
            if self.options[maf_key]:
                ...
                self.maf_readers.append(MafReader.reader_from(
        ...
        try:
            for record in o_iter:
                ...
                if processed > 0 and processed % 1000 == 0:
                    self.logger.info("Processed {0} overlapping intervals...".format(processed))
            ...
            for record in sorter:
                if counter > 0 and counter % 1000 == 0:
                    self.logger.info("Wrote {0} sorted, merged records...".format(counter))
                self.maf_writer += record
                counter += 1

            self.logger.info("Finished writing {0} sorted, merged records.".format(counter))
    ...
    def __tool_name__(cls):
        return "gdc-1.0.0-merged-protected"

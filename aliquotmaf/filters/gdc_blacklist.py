        ...
        for line in reader(source, 'rt'):
            ...
            if not head:
                ...
                assert 'tumor_aliquot_id' in head, \
                    'Required column "tumor_aliquot_id" missing from blacklist file {0}'.format(self.source)
                assert 'tag' in head, \
                    'Required column "tag" missing from blacklist file {0}'.format(self.source)
            else:
                # Parse row into dict
                ...
                tags    = row['tag'].split(';')
                assert aliquot, \
                     "Missing required field tumor_aliquot_id in blacklist file - Line {0}".format(linect)

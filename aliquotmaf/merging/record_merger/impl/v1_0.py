...
from aliquotmaf.merging.record_merger.mixins import MafMergingAverageColumnsMixin, MafMergingCombineColumnsMixin
...
class MafRecordMerger_1_0_0(BaseMafRecordMerger,
    ...
    def merge_records(self, results, tumor_only=False): 
        ...
        if results.is_singleton():
            ...
            maf_records = [self.maf_from_first_element(results, results.callers, tumor_only=tumor_only)]
            ...
            for variant in results[caller]:
                dic = {caller: [variant]}
                maf_records.append(self.maf_from_first_element(dic, results.callers, tumor_only=tumor_only))
    ...
    def maf_from_first_element(self, results, callers, star_callers=[], tumor_only=False):
        """
        ...
        for column in self.columns:
            if column in self.allele_columns() or column == 'callers':
                ...
                maf_dic[column] = get_builder(column, self.scheme, value=self.do_mean_to_int(vals))

            elif column in self.combine_columns():
                vals = self.do_uniq_list([results[i][0] for i in results if results[i]], column)
                maf_dic[column] = get_builder(column, self.scheme, value=vals)

            else:
                maf_dic[column] = selected_caller[column]

        return self.format_dic_to_record(maf_dic, callers, star_callers=star_callers, tumor_only=tumor_only)
    ...
    def collapse_by_caller_type(self, results, tumor_only=False):
        ...
        # sort by vote and select first
            ...
            other_matches = {k : results.locus_allele_map[selected_allele][k] \

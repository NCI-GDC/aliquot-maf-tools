    ...
    def annotate(self, maf_record):
        ...
        if dbsnp_rs and dbsnp_rs != ['novel']:
            ...
            for dbsnp in dbsnp_rs: 
                self.cur.execute("SELECT valstatus FROM dbsnpvalstat WHERE rsid=?", (dbsnp,))
                for row in self.cur.fetchall():
                    results.append(row)
            if results:
                validation = ';'.join([i[0] for i in sorted(list(set(results)))])
        maf_record['dbSNP_Val_Status'] = get_builder("dbSNP_Val_Status", self.scheme, value=validation)
        return maf_record 

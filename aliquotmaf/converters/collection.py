...
class InputCollection:
    ...
    def add(self, value=None, column=None, default=None, data=None):
        if data is not None:
            ...
            assert column not in self._colset, "Column '{0}' is already present!".format(column)
    ...
    def __iter__(self):
        for i in self._from_data: yield i
...
class InputData:
    ...
    def __str__(self):
        return '{0.state}: <FROM({0.value})> -> <TO({0.column} -> {0.transformed})>'.format(self)

    def __repr__(self):
        return str(self)

    def __build__(self, scheme):
        self.transformed = get_builder(self.column, scheme, value=self.value, default=self.default)
